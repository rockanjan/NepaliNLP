...
public class TokenParserRaw {
	  @SuppressWarnings({ "unchecked", "null" })
	  public List<String> readTokens(String configFile) {
	    ...
	    try {
	      ...
	      while (eventReader.hasNext()) {
	        ...
	        if (event.isStartElement()) {
	          ...
	          if (startElement.getName().getLocalPart().equalsIgnoreCase("s")) {
	        	  sentenceBuffer = new StringBuffer();	            
	          }

	          if (event.isStartElement()) {
	            if (event.asStartElement().getName().getLocalPart().equalsIgnoreCase("token")) {
